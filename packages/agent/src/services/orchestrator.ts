          ...
          if (!validation.valid) {
            ...
            try {
              ...
              if (!sanity.ok) {
                ...
                // Grammar path committed: emit events + render answer.
                yield { type: 'grammar_matched' };
                yield { type: 'sql', query: validation.sql!, explanation: 'generated by grammar compiler' };

                // V1.2 #9: programmatic render for small LIST results.
                // Bypasses the answer LLM entirely so it can't drop or truncate items.
                const intentForRender = grammarResult.intent as { primitive?: string; entity?: string } | undefined;
                const entForRender = intentForRender?.entity
                  ? this.registry.entities[this.registry.aliases[intentForRender.entity] ?? intentForRender.entity]
                  : undefined;
                const listRender = tryRenderListProgrammatically(
                  intentForRender?.primitive,
                  entForRender?.displayLabel,
                  execResult.rows,
                );
                if (listRender.ok && listRender.text) {
                  yield { type: 'token', content: listRender.text };
                  return;
                }

                // Default path: stream the answer LLM for free-form rendering.
