...
RSpec.describe SqlChatbot::Grammar::Primitives do
  ...
  it "AVG rounds to 2 places — qualified + quoted" do
  ...
  it "TOP_N uses provided rank_field, quoted" do
  ...
  it 'PG-reserved-word table name "user" is safely quoted' do
    e = SqlChatbot::Grammar::Entity.new(
      name: "user", table: "user", display_label: "User", row_count: 10,
      primary_key: "id", timestamps: {}, fields: {},
      scopes: {}, associations: {}, ranking_candidates: []
    )
    # Without quoting, FROM user resolves to PG CURRENT_USER. Silent corruption.
    expect(described_class.build(primitive: :COUNT, entity: e))
      .to eq('SELECT COUNT(*) FROM "user"')
  end
